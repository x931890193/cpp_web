...
/// @endcond
            ...
            namespace adaptor {
                ...
                struct convert<std::unordered_map<K, V, Hash, Compare, Alloc>> {
                    ...
                    operator()(msgpack::object const &o, std::unordered_map<K, V, Hash, Compare, Alloc> &v) const {
                        if (o.type != msgpack::type::MAP) { throw msgpack::type_error(); }
                        msgpack::object_kv *p(o.via.map.ptr);
                        msgpack::object_kv *const pend(o.via.map.ptr + o.via.map.size);
                ...
                struct pack<std::unordered_map<K, V, Hash, Compare, Alloc>> {
                    ...
                    msgpack::packer<Stream> &operator()(msgpack::packer<Stream> &o,
                        ...
                        o.pack_map(size);
                        for (typename std::unordered_map<K, V, Hash, Compare, Alloc>::const_iterator it(
                ...
                struct convert<std::unordered_multimap<K, V, Hash, Compare, Alloc>> {
                    msgpack::object const &
                    operator()(msgpack::object const &o, std::unordered_multimap<K, V, Hash, Compare, Alloc> &v) const {
                        if (o.type != msgpack::type::MAP) { throw msgpack::type_error(); }
                        msgpack::object_kv *p(o.via.map.ptr);
                        msgpack::object_kv *const pend(o.via.map.ptr + o.via.map.size);
                ...
                struct pack<std::unordered_multimap<K, V, Hash, Compare, Alloc>> {
                    ...
                    msgpack::packer<Stream> &operator()(msgpack::packer<Stream> &o,
                        ...
                        uint32_t size = checked_get_container_size(v.size());
                        o.pack_map(size);
                        for (typename std::unordered_multimap<K, V, Hash, Compare, Alloc>::const_iterator it(
                                v.begin()), it_end(v.end());
                             it != it_end; ++it) {
                            o.pack(it->first);
                            o.pack(it->second);
